...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = Array(Member.schema, MemberAddress.schema, MemberFollowing.schema, MemberLogin.schema, MemberService.schema, MemberStatus.schema, MemberWithdrawal.schema, NewMemberSecurity.schema, Product.schema, ProductCategory.schema, ProductStatus.schema, Purchase.schema, PurchasePayment.schema, Region.schema, ServiceRank.schema, WithdrawalReason.schema).reduceLeft(_ ++ _)
  @deprecated("Use .schema instead of .ddl", "3.0")
  def ddl = schema

  /** Entity class storing rows of table Member
   *  @param memberId Database column MEMBER_ID SqlType(INT), AutoInc, PrimaryKey
   *  @param memberName Database column MEMBER_NAME SqlType(VARCHAR), Length(100,true)
  ...
  class Member(_tableTag: Tag) extends profile.api.Table[MemberRow](_tableTag, Some("maihamadb"), "MEMBER") {
    ...
    /** Database column MEMBER_NAME SqlType(VARCHAR), Length(100,true) */
    val memberName: Rep[String] = column[String]("MEMBER_NAME", O.Length(100,varying=true))
  ...
  class MemberAddress(_tableTag: Tag) extends profile.api.Table[MemberAddressRow](_tableTag, Some("maihamadb"), "MEMBER_ADDRESS") {
    ...
    /** Uniqueness Index over (memberId,validBeginDate) (database name UQ_MEMBER_ADDRESS) */
    val index1 = index("UQ_MEMBER_ADDRESS", (memberId, validBeginDate), unique=true)
  }
  ...
  class MemberFollowing(_tableTag: Tag) extends profile.api.Table[MemberFollowingRow](_tableTag, Some("maihamadb"), "MEMBER_FOLLOWING") {
    ...
    /** Foreign key referencing Member (database name FK_MEMBER_FOLLOWING_MY_MEMBER_ID) */
    lazy val memberFk1 = foreignKey("FK_MEMBER_FOLLOWING_MY_MEMBER_ID", myMemberId, Member)(r => r.memberId, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
    /** Foreign key referencing Member (database name FK_MEMBER_FOLLOWING_YOUR_MEMBER_ID) */
    lazy val memberFk2 = foreignKey("FK_MEMBER_FOLLOWING_YOUR_MEMBER_ID", yourMemberId, Member)(r => r.memberId, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)

    /** Index over (followDatetime) (database name IX_MEMBER_FOLLOWING_DATETIME) */
    val index1 = index("IX_MEMBER_FOLLOWING_DATETIME", followDatetime)
    /** Uniqueness Index over (myMemberId,yourMemberId) (database name UQ_MEMBER_FOLLOWING) */
    val index2 = index("UQ_MEMBER_FOLLOWING", (myMemberId, yourMemberId), unique=true)
  }
  ...
  class MemberLogin(_tableTag: Tag) extends profile.api.Table[MemberLoginRow](_tableTag, Some("maihamadb"), "MEMBER_LOGIN") {
    ...
    /** Uniqueness Index over (memberId,loginDatetime) (database name UQ_MEMBER_LOGIN) */
    val index2 = index("UQ_MEMBER_LOGIN", (memberId, loginDatetime), unique=true)
  }
  ...
  class MemberService(_tableTag: Tag) extends profile.api.Table[MemberServiceRow](_tableTag, Some("maihamadb"), "MEMBER_SERVICE") {
  ...
  class MemberStatus(_tableTag: Tag) extends profile.api.Table[MemberStatusRow](_tableTag, Some("maihamadb"), "MEMBER_STATUS") {
  ...
  class MemberWithdrawal(_tableTag: Tag) extends profile.api.Table[MemberWithdrawalRow](_tableTag, Some("maihamadb"), "MEMBER_WITHDRAWAL") {
  ...
  /** Entity class storing rows of table NewMemberSecurity
   *  @param memberId Database column MEMBER_ID SqlType(INT), PrimaryKey
   *  @param loginPassword Database column LOGIN_PASSWORD SqlType(VARCHAR), Length(100,true)
   ...
   *  @param updateUser Database column UPDATE_USER SqlType(VARCHAR), Length(200,true)
   *  @param versionNo Database column VERSION_NO SqlType(BIGINT) */
  case class NewMemberSecurityRow(memberId: Int, loginPassword: String, reminderQuestion: String, reminderAnswer: String, reminderUseCount: Int, registerDatetime: java.sql.Timestamp, registerUser: String, updateDatetime: java.sql.Timestamp, updateUser: String, versionNo: Long)
  /** GetResult implicit for fetching NewMemberSecurityRow objects using plain SQL queries */
  implicit def GetResultNewMemberSecurityRow(implicit e0: GR[Int], e1: GR[String], e2: GR[java.sql.Timestamp], e3: GR[Long]): GR[NewMemberSecurityRow] = GR{
    prs => import prs._
    NewMemberSecurityRow.tupled((<<[Int], <<[String], <<[String], <<[String], <<[Int], <<[java.sql.Timestamp], <<[String], <<[java.sql.Timestamp], <<[String], <<[Long]))
  }
  /** Table description of table NEW_MEMBER_SECURITY. Objects of this class serve as prototypes for rows in queries. */
  class NewMemberSecurity(_tableTag: Tag) extends profile.api.Table[NewMemberSecurityRow](_tableTag, Some("maihamadb"), "NEW_MEMBER_SECURITY") {
    def * = (memberId, loginPassword, reminderQuestion, reminderAnswer, reminderUseCount, registerDatetime, registerUser, updateDatetime, updateUser, versionNo) <> (NewMemberSecurityRow.tupled, NewMemberSecurityRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(memberId), Rep.Some(loginPassword), Rep.Some(reminderQuestion), Rep.Some(reminderAnswer), Rep.Some(reminderUseCount), Rep.Some(registerDatetime), Rep.Some(registerUser), Rep.Some(updateDatetime), Rep.Some(updateUser), Rep.Some(versionNo)).shaped.<>({r=>import r._; _1.map(_=> NewMemberSecurityRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9.get, _10.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column MEMBER_ID SqlType(INT), PrimaryKey */
    val memberId: Rep[Int] = column[Int]("MEMBER_ID", O.PrimaryKey)
    /** Database column LOGIN_PASSWORD SqlType(VARCHAR), Length(100,true) */
    val loginPassword: Rep[String] = column[String]("LOGIN_PASSWORD", O.Length(100,varying=true))
    ...
    /** Foreign key referencing Member (database name FK_NEW_MEMBER_SECURITY_MEMBER) */
    lazy val memberFk = foreignKey("FK_NEW_MEMBER_SECURITY_MEMBER", memberId, Member)(r => r.memberId, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
  /** Collection-like TableQuery object for table NewMemberSecurity */
  lazy val NewMemberSecurity = new TableQuery(tag => new NewMemberSecurity(tag))
  ...
  class Product(_tableTag: Tag) extends profile.api.Table[ProductRow](_tableTag, Some("maihamadb"), "PRODUCT") {
  ...
  class ProductCategory(_tableTag: Tag) extends profile.api.Table[ProductCategoryRow](_tableTag, Some("maihamadb"), "PRODUCT_CATEGORY") {
  ...
  class ProductStatus(_tableTag: Tag) extends profile.api.Table[ProductStatusRow](_tableTag, Some("maihamadb"), "PRODUCT_STATUS") {
  ...
  class Purchase(_tableTag: Tag) extends profile.api.Table[PurchaseRow](_tableTag, Some("maihamadb"), "PURCHASE") {
    ...
    /** Uniqueness Index over (memberId,productId,purchaseDatetime) (database name UQ_PURCHASE) */
    val index4 = index("UQ_PURCHASE", (memberId, productId, purchaseDatetime), unique=true)
  }
  ...
  class PurchasePayment(_tableTag: Tag) extends profile.api.Table[PurchasePaymentRow](_tableTag, Some("maihamadb"), "PURCHASE_PAYMENT") {
  ...
  class Region(_tableTag: Tag) extends profile.api.Table[RegionRow](_tableTag, Some("maihamadb"), "REGION") {
  ...
  class ServiceRank(_tableTag: Tag) extends profile.api.Table[ServiceRankRow](_tableTag, Some("maihamadb"), "SERVICE_RANK") {
  ...
  class WithdrawalReason(_tableTag: Tag) extends profile.api.Table[WithdrawalReasonRow](_tableTag, Some("maihamadb"), "WITHDRAWAL_REASON") {
